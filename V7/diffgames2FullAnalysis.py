...
speedHistory = []
iteration = 0
header1 = 'V values for each object in each iterration\n | OBJECT 1 | | OBJECT 2 | | OBJECT 3 |\n'
header2 = 'V derative values for each object in each iterration\n | OBJECT 1 | | OBJECT 2 | | OBJECT 3 |\n'
header3 = 'Speed value for each object in each iterration\n | OBJECT 1 | | OBJECT 2 | | OBJECT 3 |\n'
header4 = 'State history for each object in each iterration\n | OBJECT 1 | | OBJECT 2 | | OBJECT 3 |\n'

v = open("Vfunction", "w+")
vd = open('VderFunction', 'w+')
s = open('Speed', 'w+')
state = open('States', 'w+')
...
for i in range(20):
    ...
    v.write(str(Vcurrent[0]).ljust(len(str(Vcurrent[0]))+5))
    v.write(str(Vcurrent[1]).ljust(len(str(Vcurrent[1]))+5))
    v.write(str(Vcurrent[2]).ljust(len(str(Vcurrent[2]))+5))
    v.write('\n')
    # print(Vcurrent)
    # print('--------------------')
    Vder =  np.array(Vderative(currentState, endingPoint))
    vd.write(str(Vder[0]).ljust(len(str(Vder[0]))+5))
    vd.write(str(Vder[1]).ljust(len(str(Vder[1]))+5))
    vd.write(str(Vder[2]).ljust(len(str(Vder[2]))+5))
    vd.write('\n')
    # print(Vder)
    # print('--------------------')
    Speed = Vmax * np.asarray(speed(Vder, currentState, endingPoint))
    s.write(str(Speed[0]).ljust(len(str(Speed[0]))+5))
    s.write(str(Speed[1]).ljust(len(str(Speed[1]))+5))
    s.write(str(Speed[2]).ljust(len(str(Speed[2]))+5))
    s.write('\n')
    # print(Speed)
    # print('--------------------')
    currentState = newState(currentState, Speed)
    state.write(str(currentState[0]).ljust(len(str(currentState[0]))+5))
    state.write(str(currentState[1]).ljust(len(str(currentState[1]))+5))
    state.write(str(currentState[2]).ljust(len(str(currentState[2]))+5))
    state.write('\n')
    # print(currentState)
    ...
    # Log data
    Vhistory.append(Vcurrent)
    VderHistory.append(Vder)
